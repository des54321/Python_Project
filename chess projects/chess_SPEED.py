...
b_s = 8
thinking_len = 22
piece_value = {"K": 0, "Q": 9, "B": 3, "R": 5, "P": 1, "H": 3}
...
def change(board, pos, new):
    return (
        board[: pos[0] * 16 + pos[1] * 2] + new + board[pos[0] * 16 + pos[1] * 2 + 2 :]
    )
        ...
        if kind[0] == "P":
            if kind[1] == "0":
                if in_bound([pos[0] + 1, pos[1] + 1]):
                    if get_kind(board, [pos[0] + 1, pos[1] + 1]) == str(
                        (int(kind[1]) + 1) % 2
                    ):
...
def min_max(
    ...
    if depth == 0:
        ...
        return sum(
            [
                piece_value[get_s(board, i)[0]]
                for i in all_pieces_of(board, str((int(color) + 1) % 2))
            ]
        ) - sum([piece_value[get_s(board, i)[0]] for i in all_pieces_of(board, color)])
    if maxi:
        value = -1000
        ...
        for i in all_move:
            ...
            if animation:
                b = (
                    ...
                    + "_"
                    * round((1 - (all_move.index(i) / len(all_move))) * thinking_len)
                    + "]"
                )
                print(b, end="\r")
            if og:
        ...
        for i in all_move:
            ...
            value = min(
                value,
                min_max(
                    test, depth - 1, str((int(color) + 1) % 2), True, False, alpha, beta
                ),
            )
            beta = min(beta, value)
            if value < alpha:
...
while True:
    ...
    print("")
    mabye = min_max(
        game_b, 3, "1", alpha=900, beta=1100, extra_move=False, give_score=True
    )
    print("")
    print(best_move)
    print("")
