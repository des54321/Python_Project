...
debug_color = Clerp(((0, 0, 255), (255, 0, 0), (0, 255, 0), (0, 0, 255)), (0.33, 0.66))
debug_num = 6
...
class Tile:
    ...
    def render(self, screen, fill_color, line_color, line_width=0, connects=False):
        if not fill_color == None:
            pg.draw.polygon(
                screen, fill_color, [self.screen_pos + i for i in self.points]
            )
        ...
        if connects:
            for n, i in self.connections, self.connect_type:
                pg.draw.line(
                    screen,
                    debug_color.get(i / debug_num),
                    self.screen_pos,
                    (n.screen_pos + self.screen_pos) / 2,
                    line_width // 2,
                )
    ...
    return bool(
        sum(
            [
                point_left_of_line(
                    pos,
                    tile.points[i] + tile.screen_pos,
                    tile.points[(i + 1) % len(tile.points)] + tile.screen_pos,
                )
