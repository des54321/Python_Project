...
while running:
    ...
    if key_press('c'):
        for n,i in enumerate(lines.renders):
            i:anim.Render
            if i.kind == 'tick':
                i.p1 = anim.EasePoint(lines.t,2,i.p1,on_circ(-n+9,clock_size,line_len),'sine')
                i.p2 = anim.EasePoint(lines.t,2,i.p2,on_circ(-n+9,clock_size+line_height*2,line_len),'sine')
            else:
                i.p1 = anim.EasePoint(lines.t,2,i.p1,on_circ(-n+9,clock_size+line_height,line_len),'sine')
                i.p2 = anim.EasePoint(lines.t,2,i.p2,on_circ(-n+8,clock_size+line_height,line_len),'sine')
    
    for n,i in enumerate(num_m.contents):
        if type(lines.renders[n].p1) == anim.EasePoint:
            pos = (pos_scr(lines.renders[n].p2.get(lines.t)).x/sw,pos_scr(lines.renders[n].p2.get(lines.t)).y/sh)
            i.pos = Vector2(pos)
