    ...
    def render(self,screen,fill_color,line_color,line_width=0,connects=False):
        ...
        if connects:
            for n,i in self.connections,self.connect_type:
                pg.draw.line(screen,debug_color.get(i/debug_num),self.screen_pos,(n.screen_pos+self.screen_pos)/2,line_width//2)
...
def hex_grid_l(w,h,tile_size,pos):
    ...
    for i in range(h):
        for n in range(w):
            tile_pos = Vector2(n*tile_size*1.5,(i+(n%2)*0.5)*1.732*tile_size)
