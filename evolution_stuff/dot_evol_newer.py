...
min_damage_cost = 0.005

max_damage_cost = 0.02

min_sight_cost = 0.01

max_sight_cost = 0.025
...
def evolve_trait_float(org,down,up,mini,maxi):
    return min(max(mini,org + ranran(down,up)),maxi)

def evolve_trait_down_up(org,chance,mini,maxi):
    if random() < chance:
        if random() < 0.5:
            return min(max(mini,org + 1),maxi)
        else:
            return min(max(mini,org - 1),maxi)
    else:
        return org
...
class DNA:
    ...
    def __init__(self) -> None:
        # self.repel_fore = ranran(-0.1,0.1)
        # self.repel_side = ranran(-0.1,0.1)
    ...
    def cost_photo_eval(self):
        ...
        total_cost += self.max_in_sight * in_sight_cost
    ...
    def mutate(self):
        new = deepcopy(self)
        new.attack_rate = evolve_trait_float(new.attack_rate,-4,4,5,200)
        new.damage = evolve_trait_float(new.damage,-10,10,0,1200)
        new.photo_trade_off = evolve_trait_float(new.photo_trade_off,-0.05,0.05,0,1)
        new.repel_fore = evolve_trait_float(new.repel_fore,-0.04,0.04,-2,2)
        new.repel_side = evolve_trait_float(new.repel_side,-0.04,0.04,-2,2)
        new.sight_trade_off = evolve_trait_float(new.sight_trade_off,-0.05,0.05,0,1)
        new.size = evolve_trait_float(new.size,-0.5,0.5,5,20)
        new.split_size = evolve_trait_float(new.split_size,-100,100,split_cost,100000)
        new.sight_range = evolve_trait_float(new.sight_range,-10,10,0,200)
        new.max_in_sight = evolve_trait_down_up(new.max_in_sight,0.2,0,10)
        new.color = (round(evolve_trait_float(new.color[0],-10,10,0,255)),round(evolve_trait_float(new.color[1],-10,10,0,255)),round(evolve_trait_float(new.color[2],-10,10,0,255)))
    ...
    def __str__(self) -> str:
        ...
        st += ' Max in sight:'+str(self.max_in_sight)
        return st
...
class Dot:
    ...
    def check_for_adding(self,other_dot):
        ...
        if dist < self.dna.sight_range:
            ...
            if not self.dna.max_in_sight == 0:
                if len(self.nearest_dist) < self.dna.max_in_sight:
                    self.nearest_dist.append(dist)
                    self.nearest_dots.append(other_dot)
                else:
                    if dist < max(self.nearest_dist):
                        ind = self.nearest_dist.index(max(self.nearest_dist))
                        self.nearest_dist[ind] = dist
                        self.nearest_dots[ind] = other_dot
    ...
    def update_vel(self):
        self.vel *= friction
        if not len(self.nearest_dots) == 0:
            aver_pos = Vector2(0,0)
            for i in self.nearest_dots:
                aver_pos += i.pos
            aver_pos /= len(self.nearest_dots)
            self.apply_force((self.dna.repel_fore,self.dna.repel_side),self.pos.distance_to(aver_pos)/len(self.nearest_dots),aver_pos)
...
while running:
    ...
    if run_self:
        for i in dots:
            i.apply_force((-0.7,0),900,Vector2(pg.mouse.get_pos()))
    ...
    if (not (run_self)) or key_down(pg.K_p):
        pg.display.update()
        display_on = True
    else:
        display_on = False
